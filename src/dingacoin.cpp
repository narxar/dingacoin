...
// Dingacoin: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateDingacoinNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetDingacoinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetDingacoinDustFee(tx.vout, ::minRelayTxFee);
