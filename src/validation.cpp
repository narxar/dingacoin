...
#if defined(NDEBUG)
# error "Dingacoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Dingacoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetDingacoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Dingacoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("dingacoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Dingacoin: BIP30 has been active since inception
    ...
    // Dingacoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetDingacoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Dingacoin: Disallow AuxPow blocks before it is activated.
    ...
    // Dingacoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Dingacoin: We probably want to disable this
    int nLockTimeFlags = 0;
